...
- **Vercel AI SDK + AI Gateway** para chamadas de LLM. Cada estágio pode declarar seu próprio modelo (string `"provider/model"` do Gateway), com fallback para um default por tipo de tarefa se não especificado no config profile.
...
- `model_overrides` (jsonb — modelo por estágio; chaves: `theme`, `research`, `angle`, `writer`, `hook`, `reviewer`)
- `created_at`
...
| **2. Pesquisador** | `theme` | `{ facts: [{ claim, note, unverified: true }] }` — sem busca web real nesta versão; cada fato vem marcado como conhecimento do modelo, não dado checado | `generateObject` |
...
- Busca web real no Estágio 2 (Pesquisador).
